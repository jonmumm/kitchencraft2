...
import { ModelObservableEvent, StructuredObjectModel } from "~/lib/structured-object-model";
...
export type InstantRecipeInput = {
  prompt: string;
...
export type InstantRecipeEvent = ModelObservableEvent<
  typeof INSTANT_RECIPE,
  InstantRecipeOutput
>;

export class InstantRecipeModel extends StructuredObjectModel<
  InstantRecipeInput,
  InstantRecipeOutput
> {
  ...
  protected async getUserMessage(input: InstantRecipeInput): Promise<string> {
    return `Personalization Context: ${input.personalizationContext}
    ---
    ${input.prompt}`;
  }

  protected async getSystemMessage(input: InstantRecipeInput): Promise<string> {
...
const INSTANT_RECIPE_SYSTEM_TEMPLATE = (input: InstantRecipeInput): string => `
